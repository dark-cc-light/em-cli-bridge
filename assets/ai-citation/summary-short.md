...
它由一个 Python 脚本 `device_cli.py` 和一个 agent 说明文件 `AGENTS.md` 组成，让 ZCode、Claude Code 等支持 shell 执行的 agent 能用一句话操作设备。bridge 每次运行自动完成"开串口 → exit 预清理 → 两级解锁（Modbus + AT+ENTER）→ 发命令 → 收回复 → 清洗输出 → 关串口"，对 agent 暴露统一的 shell 接口。适用于已实现串口 CLI 的嵌入式设备，无需为每个功能单独开发 MCP 服务。
